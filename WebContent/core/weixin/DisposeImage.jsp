...
{ 
	 ...
	 if('${param.editorId}' != '')
	 {
	 	   ...
	 	   var oEditor =  api.get('${param.apiId}').FCKeditorAPI.GetInstance(editorId);
	 	   
	 	   if(oEditor.EditMode == api.get('oeccd').FCK_EDITMODE_WYSIWYG)
		   {
		   		var e =  api.get('oeccd').FCKeditorAPI.GetInstance(editorId).Selection.GetSelectedElement();
		   	 
		   		 var bodyHtml =  api.get('oeccd').FCKeditorAPI.GetInstance(editorId).GetXHTML(true);
    			 ...
    			 bodyHtml= bodyHtml.replace(oldWhh, whh);
	 ...
	 {   
			...
			else
			{
				 ...
				 var prevRePath = api.get('oeccd').document.getElementById(fieldSign+'-relatePath-'+imageOrder).value;
	
				 api.get('oeccd').document.getElementById(fieldSign+'-name-show-'+imageOrder).value = name;
			 
			     api.get('oeccd').document.getElementById(fieldSign+'-cmsSysImageShowUrl-'+imageOrder).href = '${Res.url}';
				 api.get('oeccd').document.getElementById(fieldSign+'-resizeUrl-'+imageOrder).src = '${Res.resizeImgUrl}';
				 api.get('oeccd').document.getElementById(fieldSign+'-resizeUrl-'+imageOrder).title = '${Res.resName}';
				
				 api.get('oeccd').document.getElementById(fieldSign+'-resId-'+imageOrder).value = '${Res.resId}';
				 api.get('oeccd').document.getElementById(fieldSign+'-relatePath-'+imageOrder).value = '${Res.resSource}';
				 api.get('oeccd').document.getElementById(fieldSign+'-photoName-'+imageOrder).value = '${Res.resName}';
				
				 api.get('oeccd').document.getElementById(fieldSign+'-height-'+imageOrder).value = '${Res.height}';
				 api.get('oeccd').document.getElementById(fieldSign+'-width-'+imageOrder).value = '${Res.width}';
				 
				 api.get('oeccd').document.getElementById(fieldSign+'-height-show-'+imageOrder).value = '${Res.height}';
				 api.get('oeccd').document.getElementById(fieldSign+'-width-show-'+imageOrder).value = '${Res.width}';
				 
				 api.get('oeccd').document.getElementById(fieldSign+'-name-show-'+imageOrder).value = '${Res.resName}';
				 
				 //若为封面，则替换封面
				 var checkedFlag = api.get('oeccd').document.getElementById(fieldSign+'-cover-'+imageOrder).checked;
		
				 if(checkedFlag)
				 {
					api.get('oeccd').document.getElementById(fieldSign+'CmsSysImageCover').value = '${Res.resSource}';
				 }
			 }
		...
		else
		{
			...
			{
				//用于显示
				 if(api.get('oeccd').document.getElementById('${param.fieldSign}CmsSysShowSingleImage') != null)
				 {
				 	api.get('oeccd').document.getElementById('${param.fieldSign}CmsSysShowSingleImage').href = '${Res.url}';
				 }
				 
				 api.get('oeccd').document.getElementById('${param.fieldSign}CmsSysImgShow').src='${Res.resizeImgUrl}';
							 
				 api.get('oeccd').document.getElementById('${param.fieldSign}CmsSysImgShow').width = 90;
				 api.get('oeccd').document.getElementById('${param.fieldSign}CmsSysImgShow').height = 67;
			
				 api.get('oeccd').document.getElementById('${param.fieldSign}CmsSysImgWidth').value= '${Res.width}';
				 api.get('oeccd').document.getElementById('${param.fieldSign}CmsSysImgHeight').value= '${Res.height}';
							 
				 //用于cms处理
				 api.get('oeccd').document.getElementById('${param.fieldSign}').value= '${Res.resId}';
