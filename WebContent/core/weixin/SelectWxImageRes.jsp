				...
				<td align="left" valign="top">
					...
					<table id="showlist" class="listdate" width="100%" cellpadding="0" cellspacing="0">
						...
						<cms:ResInfo res="${WR.imageRes}">
							...
							<tr>
								<td>
									<input type="radio" name="checkInfo" value="${WR.wrId}" />
									<input type="hidden" id="ti-${WR.wrId}" value="${WR.resTitle}" />
								</td>
	 ...
	 var checkInfoId = $('input:radio[name=checkInfo]:checked').val();
	  ...
	  if('undefined' != typeof(checkInfoId))
	  {
		 if('${param.apiId}' == '')
		 {
		 	...
		 	W.document.getElementById('resId').value = checkInfoId;
			W.document.getElementById('msgTitle').value = '[图片'+checkInfoId+'] '+$('#ti-'+checkInfoId).val();
		 }
		 else
		 {
		 	api.get('${param.apiId}').document.getElementById('mtId').value = -1;
			api.get('${param.apiId}').document.getElementById('resId').value = checkInfoId;
			api.get('${param.apiId}').document.getElementById('msgTitle').value = '[图片'+checkInfoId+'] '+$('#ti-'+checkInfoId).val();
		 }
...
function change()
{
	...
	if('news' == resType)
	{
		window.location = 'SelectWxItemInfo.jsp?apiId=${param.apiId}';
	}
