...
{ 
	...
	var classObj = api.get(apiId).document.getElementById("classId");
	 ...
	 swfu.addPostParam("classId", api.get(apiId).document.getElementById("classId").value);
...
function uploadComplete(file) {
	//try {
	 	if (this.getStats().files_queued > 0) {
	        ...
	        //var oEditor = api.get(apiId).FCKeditorAPI.GetInstance(editorId);
			...
			{			
				var jsonObj = eval("("+serverDataArray[i]+")");
		 		...
		 		({
					...
					function(data)
					{				
						//if(oEditor.EditMode == api.get(apiId).FCK_EDITMODE_WYSIWYG)
				   		//{    
...
function uploadCompleteModule(file)
{
	...
	{				
		...
		// alert(name+":"+jsonObj.obj_0.imageUrl);
		 ...
		 W.document.getElementById(imageId).src=jsonObj.obj_0.imageUrl;
