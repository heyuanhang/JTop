...
<html xmlns="http://www.w3.org/1999/xhtml">
		...
		<cms:Class id="${param.classId}"> 
			...
			basePath='<cms:Domain/>';
			 ...
			 <cms:SystemWorkflowStep  step="${WP.currentStep}">
				...
				{
					api.title('[内容发布流程] - ${Step.stepNodeName}&nbsp;&nbsp;&nbsp; [所属栏目] - ${Class.className}' );
					
					wks = '[内容发布流程]';
				}
				else if('offline' == '${WP.flowTarget}')
				{				 
					api.title('[内容下线流程] - ${Step.stepNodeName}&nbsp;&nbsp;&nbsp; [所属栏目] - ${Class.className}');
					
					wks = '[内容下线流程]';
				}
				else if('delete' == '${WP.flowTarget}')
				{
					api.title('[内容删除流程] - ${Step.stepNodeName}&nbsp;&nbsp;&nbsp; [所属栏目] - ${Class.className}');
	...
	<body>
		<form id="userDefineContentForm" name="userDefineContentForm" method="post">
			...
			<input type="hidden" id="fromCensor" name="fromCensor" value="true"/>
					...
					<td style="text-align:center">
						...
						<table style="margin:auto;" border="0" cellspacing="0" cellpadding="0"  >
							<tr>
								...
								<td>
									<cms:QueryData objName="RC" service="cn.com.mjsoft.cms.workflow.service.WorkflowService" method="checkRecallStepUserId" var="${Info.contentId},1">
			 								<a name="btnwithicosysflag" href="javascript:javascript:recall('${Info.contentId}');"   class="btnwithico"><img src="../style/icons/script--pencil.png" width="16" height="16" /><b>撤回工作流执行&nbsp;</b> </a>																
			 						</cms:QueryData>
									...
									<cms:WorkflowActionUIHelp contentId="${param.contentId}" classId="${param.classId}" edit="true">
										...
										<cms:else>
											...
											<cms:if test="${Action.passActionName.startsWith('进入')}">
											<a name="btnwithicosysflag" onclick="javascript:submitUserDefineContentInfo('${Action.passActionName}','${Action.actionId}','${Action.fromStepId}','${Action.toStepId}');"  class="btnwithico"><img id="submitFormImg" src="../style/icons/tick.png" width="16" height="16" /><b>${Action.passActionName}&nbsp;</b> </a>
											&nbsp;&nbsp;&nbsp;
											</cms:if>
											<cms:else>
										...
										<%--<cms:if test="${Class.workflowId > 0}">
											<a href="javascript:openFlowStepSuggestDialog();"  name="btnwithicosysflag" class="btnwithico" ><img src="../style/icons/clipboard--pencil.png" width="16" height="16"/><b>流程建议&nbsp;</b> </a>
										</cms:if>
																			
									--%> 
	
									    <a name="btnwithicosysflag" href="<cms:SystemPreview id="${DInfo.contentId}"  mode="content" />"  target="_blank"  class="btnwithico"><img src="../../core/style/icons/projection-screen-presentation.png" width="16" height="16" /><b>预览&nbsp;</b> </a>
										<a href="javascript:filterCensor();"  class="btnwithico"   ><img src="../style/icon/arrow-return-180-left.png" width="16" height="16"/><b>返回&nbsp;</b> </a>
...
{	
	...
	if('${Info.censorState}' == '-1' && '${Info.creator}' != currentManager)
	{
...
{
	window.location = '<cms:BasePath/>core/console/Workbench.jsp?isReply=0&tab=2&pn=${param.pn}';
}
