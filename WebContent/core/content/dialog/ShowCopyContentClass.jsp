...
function selectTargetClass(flag)
{
    ...
    if('manage' == flag && checkedClassMap.mapSize != 0)
    {
		...
		var url = "<cms:BasePath/>content/copyContent.do?all="+allMode+"&classId="+W.currentClassId+"&contentIds="+ids+"&copyClassIds="+checkedClassMap.allKeyToString(',')+"&<cms:Token mode='param'/>";
	...
	{
    	api.get('main_content').document.getElementById('sysCopyClassCount').innerHTML = checkedClassMap.mapSize;
    	api.get('main_content').document.getElementById('copyClassIds').value = checkedClassMap.allKeyToString(',');
    	api.get('main_content').document.getElementById('sysCopyContentImg').title = '复制到:  ' + checkedClassMap.allValueToString(', ');
    }
