...
public class ClientContentListTagOLDNot_use extends TagSupport
{
    private static final long serialVersionUID = 5645397079535991773L;

    private static Logger log = Logger.getLogger( ClientContentListTagOLDNot_use.class );
    ...
    public int doStartTag() throws JspException
    {
        ...
        if( orderFlag != null && orderFlag.length > 1 )
        {
            ...
            else if( Constant.CONTENT.PUB_DATE_ORDER.equals( orderByFlag ) )
            {
                ...
                headOrderIdFlag = pageContext.getRequest().getParameter( "hof" );

                lastOrderIdFlag = pageContext.getRequest().getParameter( "lof" );
            }
        ...
        if( orderBy != null && orderWay != null )
        {
            ...
            // else
            {
                ...
                int pageAction = StringUtil.getIntValue( pageContext.getRequest().getParameter(
                    "pa" ), Constant.PAGE.PAGE_ACTION_HEAD );
                ...
                if( pageAction == Constant.PAGE.PAGE_ACTION_NEXT )
                {
                        ...
                        || pageInfo.getPageCount() == 0 )
                    {
                        ...
                        if( targetClassId == -1 )
                        {
                            if( Constant.CONTENT.VIDEO.equals( modelName ) )
                            {
                               // firstFlagId = contentService.retrieveVideoIdForEndPageMode();
                            }
                            else if( Constant.CONTENT.PHOTO.equals( modelName ) )
                            {
                               // firstFlagId = contentService.retrievePhotoIdForEndPageMode();
                            }
                            else if( Constant.CONTENT.USER_DEFINE.equals( modelName ) )
                            {
                ...
                // 取最后一页
                {
                    ...
                    else
                    {
                        ...
                        if( Constant.CONTENT.VIDEO.equals( modelName ) )
                        {
                            //firstFlagId = contentService
                                //.retrieveVideoIdForEndPageMode( targetClassId );
                        }
                        else if( Constant.CONTENT.PHOTO.equals( modelName ) )
                        {
                            //firstFlagId = contentService
                                //.retrievePhotoIdForEndPageMode( targetClassId );
                        }
                        else
                        {
                            firstOrderIdFlag = contentService
                ...
                if( !contentList.isEmpty() && !Constant.CONTENT.ADD_DATE_ORDER.equals( orderByFlag ) )
                {
                    currentHeadOrderFlag = decimalFormat.format( ( ( Map ) contentList.get( 0 ) )
                        .get( orderBy ) );
                    Object obj = ( ( Map ) contentList.get( contentList.size() - 1 ) );
                    currentLastOrderFlag = decimalFormat.format( ( ( Map ) contentList
                        .get( contentList.size() - 1 ) ).get( orderBy ) );
                }

                //pageInfo.setHeadPosQueryCondition( "&hof=" + currentHeadOrderFlag );
                //pageInfo.setLastPosQueryCondition( "&lof=" + currentLastOrderFlag );
                ...
                nextQuery = new StringBuffer( url )
                    ...
                    .append( "cp=" + pageInfo.getCurrentPage() ).append( "&pa=1" ).append(
                ...
                headQuery = new StringBuffer( url.toString() )
                    ...
                    // order
                    // ).append( "&filterBy=" + filterBy )
                    .append( "cp=" + pageInfo.getCurrentPage() ).append( "&pa=2" ).append(
                        "&hof=" + currentHeadOrderFlag ).append( "&lof=" + currentLastOrderFlag )
                    .toString();
                        ...
                        else
                        {
                            ...
                            if( url.indexOf( ".jsp" ) != -1 || url.indexOf( ".thtml" ) != -1 )
                            {
                                nextQuery = new StringBuffer( url )
                                    ...
                                    // ).append("&filterBy="
                                    // + filterBy )
                                    .append( "cp=" + pageInfo.getCurrentPage() ).append( "&pa=1" )
                    ...
                    else
                    {
                        ...
                        if( url.indexOf( ".jsp?" ) != -1 )
                        {
                            publishNextQueryChain = new StringBuffer( Constant.CONTENT.URL_SEP
                                ...
                                "&cp=" + pageInfo.getCurrentPage() ).append( "&pa=1" ).append(
        ...
        {
            long topFlag = 20000;
            double orderIdFlag = 999999999.999999;

            int limitSize = StringUtil.getIntValue( limit, 10 );

            long classId = -1;
            if( StringUtil.isStringNotNull( this.classId ) )
            {
                classId = StringUtil.getLongValue( this.classId, -1 );
            }
            ...
            // contentList = contentService
            // .retrieveLimitUserDefineContentByClassIDAndModelIdAndFlag(
            // modelBean, classId, startAddTS, endAddTS, topFlag,
            // orderIdFlag, type, censorByVar, orderBy, orderWay,
            // limitSize, limitSize,
            // Constant.PAGE.PAGE_ACTION_NEXT, 1 );
